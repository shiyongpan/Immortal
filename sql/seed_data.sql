...
INSERT INTO monsters (id, monster_name, description, monster_type, level, max_hp, attack, defense, speed, critical_rate, exp_reward, spirit_stone_reward, realm_required)
SELECT v.mid, v.mname, v.mdesc, v.mtype, v.mlv, v.mhp, v.matk, v.mdef, v.mspd, v.mcrit, v.mexp, v.mstones, r.id
FROM (VALUES
  (41, '幽冥靈蝠',   '棲息於幽冥之地的靈蝠，以陰煞之氣為食',         'normal',  70,   4200,  280, 160, 18,  8.0,  15000,   500, 4),
  (42, '寒冰玄蟒',   '身披玄冰鱗片的巨蟒，吐息帶有冰寒之氣',         'normal',  73,   4800,  295, 200, 10,  6.0,  18000,   600, 4),
  (43, '焰羽火鳳',   '鳳羽燃燒著業火，展翅遮天的火焰鳳凰',           'normal',  77,   5200,  310, 170, 14, 10.0,  22000,   750, 4),
  (44, '磐古金剛蠍', '以磐古之力凝結而成的金剛毒蠍，鉗如山嶽',       'normal',  82,   5800,  300, 230,  8,  5.0,  26000,   900, 4),
  (45, '千年鬼帥',   '修煉千年的強大鬼修，身懷陰冥秘術',             'elite',   88,  12000,  420, 260, 12, 18.0,  60000,  2500, 4),
  (46, '九頭蛟龍',   '九頭並生的古老蛟龍，每顆頭顱皆蘊含龍威',       'elite',   92,  16000,  460, 300, 10, 20.0,  90000,  3500, 4),
  (47, '元嬰真龍',   '駐守元嬰秘境的真龍，龍血可引化神突破',         'boss',    95,  35000,  600, 400, 12, 22.0, 300000, 12000, 4)
) AS v(mid, mname, mdesc, mtype, mlv, mhp, matk, mdef, mspd, mcrit, mexp, mstones, rorder)
JOIN realms r ON r.realm_order = v.rorder
ON CONFLICT (id) DO NOTHING;
...
SELECT v.mid, v.mname, v.mdesc, v.mtype, v.mlv, v.mhp, v.matk, v.mdef, v.mspd, v.mcrit, v.mexp, v.mstones, r.id
FROM (VALUES
  (51, '天魔幻影',   '化神境外圍遊蕩的天魔分身，虛實難辨',           'normal',  100,  11000,   650, 420, 20, 10.0,  100000,   3000, 5),
  (52, '雷霆龍鯤',   '身形似龍似鯤，掌控雷霆法則的遠古異獸',         'normal',  108,  12500,   700, 450, 15, 12.0,  130000,   4000, 5),
  (53, '混沌石像',   '由混沌之力凝固而成的石像，幾乎無法撼動',       'normal',  115,  13500,   680, 520,  7,  5.0,  160000,   5000, 5),
  (54, '法則侵蝕蟲', '以法則之力為食的蟲族，侵蝕一切術法防禦',       'normal',  120,  14500,   720, 460, 14, 15.0,  190000,   6000, 5),
  (55, '太古雷鵬',   '上古時期的雷鵬，身長萬里，雷霆法則化作本能',   'elite',   128,  30000,   900, 600, 18, 22.0,  500000,  18000, 5),
  (56, '虛空魔神',   '從虛空裂縫中降臨的魔神，掌控空間法則',         'elite',   138,  40000,   980, 660, 14, 25.0,  700000,  25000, 5),
  (57, '混元魔祖',   '修仙世界幕後黑手，混元之力加身的絕世強敵',     'boss',    150, 100000,  1300, 850, 11, 28.0, 2500000, 100000, 5)
) AS v(mid, mname, mdesc, mtype, mlv, mhp, matk, mdef, mspd, mcrit, mexp, mstones, rorder)
JOIN realms r ON r.realm_order = v.rorder
ON CONFLICT (id) DO NOTHING;

-- A-2：新增技能 - 元嬰境（5 個）+ 化神境（5 個），ID 31–35、41–45
INSERT INTO skills (id, skill_name, skill_name_en, description, skill_type, mp_cost, cooldown_seconds, effects, realm_required, level_required, max_level)
SELECT v.sid, v.sname, v.sname_en, v.sdesc, v.stype, v.smp, v.scd, v.seff::jsonb, r.id, v.slv, v.smaxlv
FROM (VALUES
  (31, '元嬰出竅', 'Nascent Soul Projection', '元嬰出竅遊走虛空，無視部分防禦',               'active',   60,   8, '{"damage_multiplier":2.0,"defense_ignore":0.2}',                                           '元嬰境',  70, 10),
  (32, '神識化劍', 'Divine Sense Sword',       '以神識驅使萬劍連斬，並施沉默效果',             'active',   80,  15, '{"damage_multiplier":2.5,"hits":5,"silence":true}',                                        '元嬰境',  75, 10),
  (33, '天地大挪移','Heaven Earth Shift',      '挪移天地法則，造成傷害並震暈敵人',             'active',  100,  30, '{"damage_multiplier":1.5,"stun":true,"stun_duration":2}',                                  '元嬰境',  80, 10),
  (34, '毀天滅地', 'Heaven Destroyer',         '元嬰極境神通，毀滅性的全域攻擊',               'ultimate', 150, 120, '{"damage_multiplier":4.5,"aoe":true,"defense_ignore":0.5}',                                '元嬰境',  85,  5),
  (35, '涅槃重生', 'Nirvana Rebirth',          '瀕死時元嬰自燃涅槃，以 30% HP 重生一次',      'passive',    0,   0, '{"revive":true,"revive_hp_rate":0.3,"revive_cooldown_seconds":600}',                      '元嬰境',  90, 10),
  (41, '法則之力', 'Law Dominance',            '掌握一絲法則之力，穿透半數防禦',               'active',   80,  10, '{"damage_multiplier":2.2,"defense_ignore":0.5}',                                           '化神境', 100, 10),
  (42, '領域展開', 'Domain Expansion',         '展開化神領域，全面提升攻守並施以持續傷害',     'ultimate', 200, 180, '{"domain":true,"attack_buff":0.5,"defense_buff":0.5,"dot_multiplier":1.5,"duration":5}',  '化神境', 110,  5),
  (43, '神魂滅殺', 'Soul Annihilation',        '直擊敵魂，造成真實傷害並施沉默效果',           'active',  120,  25, '{"damage_multiplier":3.0,"true_damage":true,"silence_duration":3}',                        '化神境', 120, 10),
  (44, '天地同壽', 'Eternal Longevity',        '與天地共存，大幅提升生命上限並持續回復',       'passive',    0,   0, '{"max_hp_bonus":0.5,"hp_regen_rate":0.05}',                                                '化神境', 130, 10),
  (45, '逆天改命', 'Fate Reversal',            '逆轉天道，絕境中以 50% HP 復生並反傷',         'ultimate', 300, 999, '{"revive":true,"revive_hp_rate":0.5,"reflect_damage_multiplier":2.0,"invincible_duration":2}','化神境', 140,  1)
) AS v(sid, sname, sname_en, sdesc, stype, smp, scd, seff, realm_name, slv, smaxlv)
JOIN realms r ON r.realm_name = v.realm_name
ON CONFLICT (id) DO NOTHING;

-- A-3：新增主線任務（ID 6–9）
INSERT INTO quests (id, quest_name, description, quest_type, realm_required, level_required, prerequisite_quest_id, is_repeatable, repeat_cooldown_hours)
SELECT v.qid, v.qname, v.qdesc, 'main', r.id, v.qlv, v.qpre, false, 0
FROM (VALUES
  (6, '金丹磨礪', '在金丹境的磨礪中積累力量，斬殺試煉妖獸後突破元嬰', 50, 5, '金丹境'),
  (7, '元嬰初啟', '元嬰境修行初始，清除盤踞幽冥之地的靈蝠與鬼帥',     70, 6, '元嬰境'),
  (8, '真龍討伐', '元嬰境終極考驗，討伐元嬰真龍並收取龍血引化神突破', 80, 7, '元嬰境'),
  (9, '化神問道', '踏入化神境，以廝殺問道天地，斬殺混元魔祖揭示真相', 100, 8, '化神境')
) AS v(qid, qname, qdesc, qlv, qpre, realm_name)
JOIN realms r ON r.realm_name = v.realm_name
ON CONFLICT (id) DO NOTHING;
...
INSERT INTO quest_steps (quest_id, step_order, description, step_type, target_id, target_name, required_count) VALUES
-- 金丹磨礪
(6, 1, '斬殺金甲犀牛，以其魂魄淬煉金丹',          'kill',        21, '金甲犀牛', 10),
(6, 2, '斬殺九尾天狐，從靈核中感悟幻術法則',       'kill',        23, '九尾天狐',  1),
(6, 3, '完成突破，成就元嬰初成',                   'reach_realm',  4, '元嬰境',    1),
-- 元嬰初啟
(7, 1, '清除幽冥靈蝠巢穴',                         'kill',        41, '幽冥靈蝠', 10),
(7, 2, '斬殺千年鬼帥真身',                         'kill',        45, '千年鬼帥',  3),
-- 真龍討伐
(8, 1, '清掃元嬰真龍的前哨蛟龍族群',               'kill',        46, '九頭蛟龍',  5),
(8, 2, '討伐元嬰真龍，收取龍血',                   'kill',        47, '元嬰真龍',  1),
(8, 3, '以龍血為引，完成化神突破',                 'reach_realm',  5, '化神境',    1),
-- 化神問道
(9, 1, '以廝殺感悟雷霆法則',                       'kill',        55, '太古雷鵬',  3),
(9, 2, '領悟空間法則之道',                         'kill',        56, '虛空魔神',  2),
(9, 3, '斬殺混元魔祖，揭示幕後真相',               'kill',        57, '混元魔祖',  1)
ON CONFLICT DO NOTHING;
